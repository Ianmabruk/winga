  ...
  return (
    <div className="grid gap-4">
      <article className="rounded-2xl border border-white/50 bg-white/80 p-4 shadow-glass">
        ...
        >
          <input className="rounded-xl border border-skybrand-200 px-3 py-2" placeholder="Full name" value={form.fullName} onChange={(event) => setForm((prev) => ({ ...prev, fullName: event.target.value }))} />
          <input className="rounded-xl border border-skybrand-200 px-3 py-2" placeholder="Email" value={form.email} onChange={(event) => setForm((prev) => ({ ...prev, email: event.target.value }))} />
          ...
          <div className="flex gap-2">
            ...
            <button className="rounded-xl bg-skybrand-500 px-4 py-2 text-sm font-semibold text-white">Add</button>
          </div>
        </form>
      </article>
